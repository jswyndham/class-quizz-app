...
export const updateQuiz = async (req, res) => {
  try {
    ...
    // Update ClassGroups only if there are changes
    const oldClasses = new Set(existingQuiz.class.map(String));
    const newClasses = new Set(classIds.map(String));

    const classesToRemove = [...oldClasses].filter((x) => !newClasses.has(x));
    const classesToAdd = [...newClasses].filter((x) => !oldClasses.has(x));

    // Remove quiz from old classes
    await Promise.all(
      classesToRemove.map((classId) =>
        ClassGroup.findByIdAndUpdate(classId, {
