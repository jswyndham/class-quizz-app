...
const stripImages = (text) => {
  // This regex is a basic example and might need to be adjusted based on your exact requirements
...
export const validateQuizIdParam = withValidationErrors([
  ...
  param("id").custom(async (value, { req }) => {
    ...
    if (!quiz)
      throw new NotFoundError(`Quiz with id ${value} could not be found.`);
  ...
  body("questions.*.questionText")
    ...
    .custom((text) => {
      ...
      if (textWithoutImages.length > 1000) {
        throw new Error("Question text is too long when excluding images");
      }
      return true;
    }),
