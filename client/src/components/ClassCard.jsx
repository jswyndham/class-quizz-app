...
const ClassCard = ({ className, subject, school, classStatus }) => {
  ...
  useEffect(() => {
    const checkOutsideMenu = (e) => {
      if (showClassCardMenu && ref.current && !ref.current.contains(e.target)) {
  ...
  return (
    ...
    >
      ...
      <div ref={ref} className="flex flex-row-reverse -mt-60 mr-12 z-10">
        <ClassCardMenu />
      </div>
    </ClassCardContext.Provider>
  );
